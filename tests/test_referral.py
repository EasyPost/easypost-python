...
@pytest.mark.vcr()
def test_referral_add_payment_method(referral_customer_prod_api_key):
    ...
    """
    with pytest.raises(Exception) as error:
        easypost.Referral.add_payment_method(
...
@pytest.mark.vcr()
def test_referral_refund_by_amount(referral_customer_prod_api_key):
    """This test requires a referral customer user's production API key via REFERRAL_CUSTOMER_PROD_API_KEY.

    We expect this test to fail because we don't have valid billing details to use. Assert the correct error.
    """
    with pytest.raises(Exception) as error:
        easypost.Referral.refund_by_amount(refund_amount="$20.00")

    assert str(error.value) == "Refund amount doesn't match. Please escalate this to finance, this is unexpected."


@pytest.mark.vcr()
def test_referral_refund_by_payment_log(referral_customer_prod_api_key):
    """This test requires a referral customer user's production API key via REFERRAL_CUSTOMER_PROD_API_KEY.

    We expect this test to fail because we don't have valid billing details to use. Assert the correct error.
    """
    with pytest.raises(Exception) as error:
        easypost.Referral.refund_by_payment_log(payment_log_id="paylog_123")
